...
手册的教学法也有讲究：口吻是对话体，骨子是参与式——每个机制揭晓前强制你先猜（猜错一次比听懂十次记得牢，这条是构建层硬约束，缺了直接构建失败），底气是证据体——每个论断后面跟着原文引用和真实样本。
...
构建层强制、不靠自觉的事：症状缺证据标记 → 拒绝；有难点的站缺预测点 → 拒绝；预览卡指向不存在的章节 → 拒绝；引用的 SVG 不存在 → 拒绝。
